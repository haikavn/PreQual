...
-- =============================================
CREATE PROCEDURE [dbo].[ReportBuyersComparison]
(
...
BEGIN
	...
	select c.Id as CampaignId, 
	   ...
	   from LeadMainReportDay rd 
			  ...
			  left join Buyer b on b.Id = rd.BuyerId
			  left join BuyerChannel bc on bc.Id = rd.BuyerChannelId
			  where (@buyerid = 0 or rd.BuyerId = @buyerid) and (@campaignid = 0 or rd.CampaignId = @campaignid) and (rd.Created = @date1 or rd.Created = @date2 or rd.Created = @date3)
			  group by 
			  rd.CampaignId, rd.AffiliateId, rd.AffiliateChannelId, b.Id, b.Name, c.Id, c.Name, a.Id, a.Name, ac.Id, ac.Name, rd.Created
			  order by rd.Created
END