...
namespace TheArtOfDev.HtmlRenderer.PdfSharp
{
    ...
    public static PdfDocument SplitHtmlIntoPagedPdf(string html, string pageBreakBeforeClass, PdfGenerateConfig config, PdfDocument pdf)
        {

            var splittedData=System.Text.RegularExpressions.Regex.Split(html, pageBreakBeforeClass);

            var beginOf = splittedData[0];
            var endOf = splittedData[splittedData.Length - 1];

            for (var i=1; i<splittedData.Length-1; i++)
            {
                var content = beginOf + splittedData[i] + endOf;
                AddPageToPdf(content, config, ref pdf);
        ...
        private static void AddPageToPdf(string html, PdfGenerateConfig config, ref PdfDocument pdf)
        {
            var nodeDoc = PdfGenerator.GeneratePdf(html, config);
