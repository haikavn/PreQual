...
namespace Adrack.WebApi.Helpers
{
    ...
    public static class Utils
    {
        ...
        public static void AddXmlElement(XmlDocument xmlDocument, string parentName, string elementName)
        {
            if (string.IsNullOrEmpty(parentName))
                parentName = "request";

            XmlNodeList nodes = xmlDocument.GetElementsByTagName(parentName);
            if (nodes.Count > 0)
            {
                if (nodes[0][elementName] == null)
                    nodes[0].AppendChild(xmlDocument.CreateElement(elementName));
            }
            else
            {
                XmlElement parentElement = xmlDocument.CreateElement(parentName);
                parentElement.AppendChild(xmlDocument.CreateElement(elementName));
                xmlDocument.DocumentElement.AppendChild(parentElement);
            }
        }

        public static void ClearXmlElements(XmlDocument xmlDocument, List<string> except = null)
        {
            if (except == null)
            {
            ...
            else
            {
                ...
                foreach(XmlNode node in nodes)
                {
                    ...
                    {
                        string name = node.Name + "," + node.ParentNode.Name;
