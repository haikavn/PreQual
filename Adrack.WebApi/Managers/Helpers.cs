...
namespace Adrack.WebApi.Helpers
{
    /// <summary>
    ///     Class Helpers.
    /// </summary>
    public class Helpers
    {
        ...
        public static string Post(string url, string data, string contentType = "application/x-www-form-urlencoded", string method = "POST", Dictionary<string, string> postingHeaders = null)
        {
        ...
        {
            var queryParameters = context.HttpRequest.RequestUri.ParseQueryString();
            ...
            foreach (string key in queryParameters.Keys)
            {
                ...
                if(nodeList.Count > 0)
                {
                    nodeList[0].InnerText = queryParameters[key].ToString();
                }
            }
        }
        ...
        public static Dictionary<string, string> ReadPostingHeaders(BuyerChannel buyerChannel)
        {
            ...
            if (!string.IsNullOrEmpty(buyerChannel.PostingHeaders))
            {
                if (buyerChannel.DataFormat != (short)PostFormat.SOAP)
                {
                    ...
                    {
                        var values = h.Split(new string[1] { "[=]" }, StringSplitOptions.None);
                        if (values.Length >= 2)
                        {
                            var s = "";
