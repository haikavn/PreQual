...
{ 
    ...
    public class RequestManager : IDisposable
    {
        ...
        public bool ProcessData(HttpRequestMessage httpRequest, RequestContext context,
            bool isachannel)
        {
            ...
            /*if (!context.CachedUrlService.CheckCachedUrl(baseUrl))
            {
                ...
                SharedData.ResetBuyerChannelLeadsCount();
            }*/
            ...
            var queryParameters = httpRequest.RequestUri.ParseQueryString();
            

            if (!string.IsNullOrEmpty(queryParameters["mode"]))
                context.Extra["mode"] = queryParameters["mode"];
            else
                context.Extra["mode"] = "";

            if (!string.IsNullOrEmpty(queryParameters["BuyerChannelId"]))
            {
                long buyerChannelId = 0;
                long.TryParse(queryParameters["BuyerChannelId"], out buyerChannelId);
                context.Extra["BuyerChannelId"] = buyerChannelId;
            }
            else
            {
            ...
            if (context.Extra.ContainsKey("informat"))
            {
                    ...
                    (short)RequestResult.StatusTypes.Test)
                {
                    ...
                    if (export.Result.ResultType != RequestResult.ResultTypes.Success && affiliateChannel.Timeout.HasValue && affiliateChannel.Timeout.Value > 0 && export.PastTotalSeconds >= affiliateChannel.Timeout.Value)
                    {
