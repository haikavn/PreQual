...
$(document).ready(function () {
	...
	urlItems.forEach(item => {
		...
		if (splited[0] == 'fname') $('#FirstName').val(item.split('=')[1].replace('+', ' '));
			...
			if ($('#ZIP').val().length == 5 && validateZIP($('#ZIP').val())) GetGeoDataByZip($('#ZIP').val(), '');
	...
	$('#ContinueBtn').on('click', function() {
		if ($('.step-' + step).hasClass('validate')) {
			...
			if (customStep != 0 && step == 13) step = customStep;
...
function CountClick() {
	$.ajax({
		type: 'POST',
		url: 'https://proffiliant.adrack.com/home/click?key=d4796502-df3d-4a6b-8612-1b0a2b5641c5&type=1',
		async: true,
		success: function(response) {
