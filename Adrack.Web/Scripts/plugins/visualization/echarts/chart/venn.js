    ...
    function (e) {
        ...
        return t.prototype = {
            ...
            _buildVenn: function (e) {
                ...
                a[0].value > a[1].value
                    ? (t = this.zr.getHeight() / 3, i = t * Math.sqrt(a[1].value) / Math.sqrt(a[0].value))
                    : (i = this.zr.getHeight() / 3, t = i * Math.sqrt(a[0].value) / Math.sqrt(a[1].value));
                var o = this.zr.getWidth() / 2 - t,
                    r = (t + i) / 2 * Math.sqrt(a[2].value) / Math.sqrt((a[0].value + a[1].value) / 2),
                    s = t + i;
                0 !== a[2].value &&
                    (s = this._getCoincideLength(a[0].value, a[1].value, a[2].value, t, i, r, Math.abs(t - i), t + i));
                var l = o + s, h = this.zr.getHeight() / 2;
                if (this._buildItem(e, 0, a[0], o, h, t), this._buildItem(e, 1, a[1], l, h, i), 0 !== a[2].value &&
                    a[2].value !== a[0].value &&
                    a[2].value !== a[1].value) {
                    var m = (t * t - i * i) / (2 * s) + s / 2,
                        ...
                        d = 0,
                        p = 0;
                    a[0].value > a[1].value && o + m > l && (p = 1), a[0].value < a[1].value && o + V > l && (d = 1),
                        this._buildCoincideItem(e, 2, a[2], o + m, h - U, h + U, t, i, d, p);
                }
            },
            ...
            _buildCoincideItem: function (e, t, i, n, a, r, l, h, m, V) {
                ...
                k = new o(k), k.buildPathArray && (k.style.pathArray = k.buildPathArray(f.path)), s.pack(k,
                    U[e],
                    0,
                    i,
                    t,
                    i.name), this.shapeList.push(k);
            },
            ...
            getLabelText: function (e, t, i) {
                ...
                return o
                    ? "function" == typeof o
                        ? o(a.name, t.name, t.value)
                        : "string" == typeof o
                            ? (o = o.replace("{a}", "{a0}").replace("{b}", "{b0}").replace("{c}", "{c0}"), o =
                                o.replace("{a0}", a.name).replace("{b0}", t.name).replace("{c0}", t.value))
                            : void 0
                    : t.name;
            },
            ...
            return s.prototype = {
                        ...
                        for (; d.length > 0 && !isNaN(d[0]);) {
                            ...
                            switch (U) {
                                ...
                                case "t":
                                    ...
                                    break;
                                case "A":
                                    g = d.shift(), b = d.shift(), f = d.shift(), k = d.shift(), _ = d.shift(), W = h, X = m, h =
                                        d.shift(), m = d.shift(), x = "A", L = this._convertPoint(W, X, h, m, k, _, g, b, f);
                                    break;
                                case "a":
                ...
                _convertPoint: function (e, t, i, n, a, s, l, h, m) {
                    ...
                    var u = c * l * d / h,
                        ...
                        _ = [(-1 * U - u) / l, (-1 * d - y) / h],
                        x = r(k, _);
                    return o(k, _) <= -1 && (x = Math.PI), o(k, _) >= 1 && (x = 0), 0 === s && x > 0 && (x -= 2 * Math.PI),
                        ...
                        switch (d) {
                            ...
                            case "A":
                                var p = m[0],
                                    ...
                                    x = u > y ? 1 : u / y,
                                    L = u > y ? y / u : 1;
                                e.translate(p, c), e.rotate(f), e.scale(x, L), e.arc(0, 0, _, g, g + b, 1 - k), e.scale(1 / x,
