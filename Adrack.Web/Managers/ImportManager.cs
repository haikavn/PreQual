...
namespace Adrack.Managers
{
    ...
    public class ImportManager
    {
        ...
        private bool ValidateValue(RequestContext context, CampaignField tplfield, string val, out string result)
        {
            result = "";
            ...
            switch (tplfield.Validator)
            {
                ...
                case (short)Validators.DateTime:
                    if (Validator.IsValidDateTime(val, tplfield.ValidatorSettings, out dob))
                    {
                        return true;
                    }
                    else
                ...
                case (short)Validators.DateOfBirth:
                    var res = Validator.IsValidDateTime(val, tplfield.ValidatorSettings, out dob);
                    if (res)
                    {
                ...
                case (short)Validators.Decimal:
                    {
                        ...
                        var after = 2;
                        if (str.Length > 0)
                            int.TryParse(str[0], out before);

                        if (str.Length > 1)
                            int.TryParse(str[1], out after);
            ...
            foreach (XmlNode node in parent.ChildNodes)
                if (node.NodeType == XmlNodeType.Element)
                    if (node.ParentNode != null)
                    {
                        ...
                        var pi = lead.GetType().GetProperty(tplfield.DatabaseField);
            ...
            AffiliateChannel format, LeadMain lead, LeadContent leadContent)
        {
            ...
            var fields = GetFields(context, format.Id);
            ...
            {
                if (f.CampaignTemplateId <= 0) continue;

                var tplfield = context.CampaignTemplateService.GetCampaignTemplateById(f.CampaignTemplateId, true);

                if (tplfield == null) continue;

                nodelist = XmlHelper.GetElementsByTagName(dataxml, f.TemplateField);

                if (nodelist == null && tplfield.Required)
                {
                    result.Set(RequestResult.ResultTypes.Error, RequestResult.ErrorTypes.MissingField,
                        "XML field '" + f.TemplateField + "' is missing");
                    return null;
                }
                ...
                if (nodelist != null &&
                    (nodelist.ParentNode == null && nodelist.Name == f.SectionName ||
                    nodelist.ParentNode != null && nodelist.ParentNode.Name == f.SectionName))
                    datael = nodelist;
                ...
                if (!context.MinProcessingMode && !ValidateValue(context, tplfield,
                        string.IsNullOrEmpty(f.DefaultValue) ? value.ToString() : f.DefaultValue, out validationError))
                {
                ...
                if (!context.MinProcessingMode && !ValidateFilter(context, tplfield, f, value) ||
                ...
                if (dbname.Length == 0) continue;

                pi = leadContent.GetType().GetProperty(tplfield.DatabaseField);
                    ...
                    try
                    {
                        ...
                        else if (t == typeof(DateTime))
                        {
                            if (tplfield.Validator == (short)Validators.DateOfBirth ||
                                tplfield.Validator == (short)Validators.DateTime)
                                value = DateTime.ParseExact(value.ToString(), (string.IsNullOrEmpty(tplfield.ValidatorSettings) ? "MM/dd/yyyy" : tplfield.ValidatorSettings),
            ...
            out string AffiliateXmlField, out Campaign campaign, out AffiliateChannel format)
        {
            ...
            if (context.HttpRequest.HttpMethod.ToLower() == "post")
            {
            ...
            {
                affiliateChannelId = context.HttpRequest.QueryString[AffiliateXmlField];
            }
            ...
            if (campaign != null)
            {
                if (context.HttpRequest.HttpMethod.ToLower() == "get")
                {
