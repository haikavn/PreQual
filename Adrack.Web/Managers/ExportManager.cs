...
namespace Adrack.Managers
{
    ...
    public class ExportManager : IDisposable
    {
        ...
        ///     Ping tree
        /// </summary>
        private List<BuyerChannel> buyerChannels = new List<BuyerChannel>();
        ...
        protected string GenerateResponse(HttpRequestBase request, short pingTreeTestMode, bool hasChild)
        {
            string[] responses =
            {
            ...
            if (i == 1)
            {
                switch (pingTreeTestMode)
                {
                    //always reject
                    case 5:
                    case 0: responses[i] = "<Response><status>reject</status><redirect>" + Helpers.GetBaseUrl(request) + "</redirect></Response>"; break;
                    ...
                    case 4:
                        if (hasChild)
                        {
                            ...
                            responses[i] = "<Response><status>reject</status><redirect>" + Helpers.GetBaseUrl(request) + "</redirect></Response>";
                        break;
                    //random price reject
                    case 2:
                        n = rnd.Next(0, 1);
        ...
        private BuyerChannel[] GetBuyerChannels(RequestContext context, LeadMain l, LeadContent leadContent)
        {
        ...
        /// <param name="lead">The lead.</param>
        private bool SetXMLDataValue(RequestContext context, object dataobj, CampaignField ct,
            BuyerChannelTemplate f, string value, LeadMain lead, Buyer buyer, bool processWithoutIntegration = false)
        {
            ...
            if (ct != null)
            {
                if (!MappingFields.ContainsKey(ct.TemplateField))
                    MappingFields[ct.TemplateField] = f.TemplateField;
            }
            ...
            if (string.IsNullOrEmpty(f.DefaultValue))
            {
                var matchings = (List<BuyerChannelTemplateMatching>)context.BuyerChannelTemplateMatchingService
                    .GetBuyerChannelTemplateMatchingsByTemplateId(f.Id);
                        ...
                        if (context.AllFieldValues.ContainsKey(ct.TemplateField))
                        {
                            ...
                            switch (ct.TemplateField.ToLower())
                            {
                                case "addressmonth":
                                    ...
                                    {
                                        switch(f.TemplateField.ToLower())
                                        {
            ...
            else
            {
                ...
                else if (f.DefaultValue.ToLower() == "#leadid#")
                {
                    context.Extra["LeadIdFieldCreated"] = true;
                    AppendLeadIdField(context, datafield, dataobj as XmlDocument, lead, buyer);
                }
                else if (f.DefaultValue.ToLower() == "[optional]")
                {
                    if (dataobj != null)
                    {
                        var elementsToRemove = (dataobj as XmlDocument).GetElementsByTagName(f.TemplateField);
                        foreach (XmlElement element in elementsToRemove)
                        {
                            if (element.ParentNode != null && element.ParentNode.Name == f.SectionName)
                            {
                ...
                else if (f.DefaultValue.ToLower() == "[required]")
                {
                ...
                {
                    datafield.InnerText = f.DefaultValue;
                    removeFromHashedFields = true;
                }
            }
                ...
                foreach (FilterData filterData in filterDataList)
                {
                    if (!ValidateFilterCondition(context, filterData.CampaignField, (BuyerChannelFilterCondition)filterData.Filter, filterData.Value))
                    {
                        if (result.ResultType != RequestResult.ResultTypes.Success)
                        {
                            ...
                            result.Set(RequestResult.ResultTypes.Reject, RequestResult.ErrorTypes.FilterError,
                            ...
                            if (campaign.CampaignType == (short)CampaignTypes.LeadCampaign)
                            {
                                ...
                                InsertLeadMainResponse(context, buyerChannel, lead, leadContent, Helpers.UtcNow(),
                                    (Helpers.UtcNow() - utcNow).TotalMilliseconds,
                                    "F: " + ((BuyerChannelTemplate)filterData.ChannelField).TemplateField + " '" + filterData.Value + "' does not match the criteria",
                                    "F: " + ((BuyerChannelTemplate)filterData.ChannelField).TemplateField + " '" + filterData.Value + "' does not match the criteria", 0, 0,
                                    RequestResult.ResultTypes.Reject,
                                    RequestResult.ErrorTypes.FilterError, filterData.CampaignField.Validator);
                            }
        ...
        protected BuyerChannel ProcessTask(RequestContext context, LeadMain lead, LeadContent leadContent, LeadMainResponse ls,
            BuyerChannel buyerChannel, Campaign campaign, Buyer buyer, decimal lAffiliatePrice,
            XmlDocument xmldoc, List<string> nodesToHash, List<string> nodesToHashInSent)
        {
            ...
            if ((PastSeconds < buyerChannel.Timeout + buyerChannel.AfterTimeout || buyerChannel.Timeout == 0) && (!affiliateChannel.Timeout.HasValue || affiliateChannel.Timeout.Value == 0 || PastTotalSeconds < affiliateChannel.Timeout.Value))
            {
                ignoreTimeout = true;
            ...
            {
                var postingHeaders = new Dictionary<string, string>();
                var postingHeadersStr = buyerChannel.PostingHeaders;
               

                if (!string.IsNullOrEmpty(buyerChannel.PostingHeaders))
                {
                    if (buyerChannel.DataFormat != (short)PostFormat.SOAP)
                    {
                        var headers = buyerChannel.PostingHeaders.Split(new string[1] { "\r\n" }, StringSplitOptions.None);
                        foreach (var h in headers)
                        {
                            ...
                            if (values.Length >= 2)
                            {
                                ...
                                for (var n = 1; n < values.Length; n++)
                                    s += values[n];
                                postingHeaders[values[0]] = s;
                            }
                        }
                    }
                }

                if (ignoreTimeout || ((PastSeconds < buyerChannel.Timeout + buyerChannel.AfterTimeout || buyerChannel.Timeout == 0) && (!affiliateChannel.Timeout.HasValue || affiliateChannel.Timeout.Value == 0 || PastTotalSeconds < affiliateChannel.Timeout.Value)))
                {
                    ...
                    if (affiliateChannel.Timeout.HasValue && affiliateChannel.Timeout.Value > 0)
                        timeout = Math.Min(timeout, affiliateChannel.Timeout.Value);

                    ignoreTimeout = true;
                    if (buyerChannel.PostingUrl != "auto")
                        response = Helpers.PostXml((string.IsNullOrEmpty(CustomPriceRejectUrl) ? buyerChannel.PostingUrl : CustomPriceRejectUrl), dataToSend,
                            (timeout) * 1000,
            ...
            if (ignoreTimeout || ((PastSeconds < buyerChannel.Timeout + buyerChannel.AfterTimeout || buyerChannel.Timeout == 0) && (!affiliateChannel.Timeout.HasValue || affiliateChannel.Timeout.Value == 0 || PastTotalSeconds < affiliateChannel.Timeout.Value)))
            {
                ...
                if (campaign.CampaignType == (short)CampaignTypes.LeadCampaign)
                {
                    ...
                    var BuyerPrice = buyerChannel.BuyerPriceOption == 0 ? buyerChannel.BuyerPrice : responsePrice;
                    short AffiliatePriceOption = buyerChannel.AffiliatePriceOption;
                    decimal lAffiliatePrice2 = lAffiliatePrice = buyerChannel.AffiliatePrice;

                    if (AffiliatePriceOption == (short)AffiliateChannelPriceMethods.TakeFromAffilate)
                    {
                    ...
                    {
                        if (BuyerPrice - BuyerPrice *
                            affiliateChannel.NetworkTargetRevenue / 100 >=
                            leadContent.Minprice)
                            lAffiliatePrice = BuyerPrice *
                                              (100 - affiliateChannel
                                               .NetworkTargetRevenue) / 100;
                        else
                            lAffiliatePrice = (decimal)leadContent.Minprice;
                    }
                    else
                    {
                        if (lAffiliatePrice2 >= leadContent.Minprice)
                            lAffiliatePrice = lAffiliatePrice2;
                        else
                            lAffiliatePrice = (decimal)leadContent.Minprice;
                    }
                    ...
                    if (PastSeconds > buyerChannel.Timeout && PastSeconds <= buyerChannel.AfterTimeout && result.ErrorType != RequestResult.ErrorTypes.RequestTimeoutError)
                    ...
                    if (!parsedFromCustomPriceRejectLabel)
                        if (buyerChannels.Count > 0 && !priceRejectLocks.ContainsKey(buyerChannel.Id))// && responsePrice == 0) //remove channel if no price reject
                            buyerChannels.RemoveAt(0);
                }
                else
                {
            ...
            decimal AffiliatePrice, XmlDocument campaignXml)
        {
            ...
            foreach (var f in buyerChannelTemplates)
            {
                ...
                {
                    if (!SetXMLDataValue(context, dataobj, ct, f, dataElValue, lead, buyer, true))
                    {
                        if (result.ResultType != RequestResult.ResultTypes.Success)
                        {
                            var res = result.ResultType;
                            ...
                            if (campaign.CampaignType == (short)CampaignTypes.LeadCampaign)
                            {
                                ...
                                InsertLeadMainResponse(context, buyerChannel, lead, leadContent, Helpers.UtcNow(),
                                    (Helpers.UtcNow() - utcNow).TotalMilliseconds,
                                    "Field '" + f.TemplateField + "' is required",
                                    "Field '" + f.TemplateField + "' is required", 0, 0,
                                    RequestResult.ResultTypes.Reject,
                                    RequestResult.ErrorTypes.FilterError, ct.Validator);
                            }
                    ...
                    if (!string.IsNullOrEmpty(subIdValue) && context.SubIdWhiteListService.CheckSubIdWhiteList(subIdValue, buyerChannel.Id) == 0)
                    {
                        if (result.ResultType != RequestResult.ResultTypes.Success)
                        {
                            ...
                            if (campaign.CampaignType == (short)CampaignTypes.LeadCampaign)
                            {
                                ...
                                InsertLeadMainResponse(context, buyerChannel, lead, leadContent, Helpers.UtcNow(),
                                    (Helpers.UtcNow() - utcNow).TotalMilliseconds,
                                    "Field '" + datael.Name + "' did not pass sub id white list",
                                    "Field '" + datael.Name + "' did not pass sub id white list", 0, 0,
                                    RequestResult.ResultTypes.Reject,
                                    RequestResult.ErrorTypes.FilterError, ct.Validator);
                            }
                    ...
                    if (!ValidateFilter(context, ct, f, dataElValue))
                    {
                        if (result.ResultType != RequestResult.ResultTypes.Success)
                        {
                            ...
                            if (campaign.CampaignType == (short)CampaignTypes.LeadCampaign)
                            {
                                ...
                                InsertLeadMainResponse(context, buyerChannel, lead, leadContent, Helpers.UtcNow(),
                                    (Helpers.UtcNow() - utcNow).TotalMilliseconds,
                                    f.TemplateField + ": '" + dataElValue + "' does not match the criteria",
                                    f.TemplateField + ": '" + dataElValue + "' does not match the criteria", 0, 0,
                                    RequestResult.ResultTypes.Reject,
                                    RequestResult.ErrorTypes.FilterError, ct.Validator);
                            }
                ...
                SetXMLDataValue(context, dataobj, ct, f, dataElValue, lead, buyer);
            }
            ...
            if (dataobj == null && campaign.CampaignType == (short)CampaignTypes.LeadCampaign)
            {
                if (result.ResultType != RequestResult.ResultTypes.Success)
                {
                    ...
                    InsertPostedData(context, lead, leadContent, buyerChannel, "", RequestResult.ResultTypes.Reject);
                    InsertLeadMainResponse(context, buyerChannel, lead, leadContent, Helpers.UtcNow(),
                        (Helpers.UtcNow() - utcNow).TotalMilliseconds, "Check the buyer channel integration",
                        "Check the buyer channel integration", 0, 0, RequestResult.ResultTypes.Reject,
                        RequestResult.ErrorTypes.None, null);
            ...
            {
                buyerChannel = ProcessTask(context, lead, leadContent, ls, buyerChannel, campaign, buyer, AffiliatePrice,
                    dataobj == null ? null : dataobj as XmlDocument, nodesToHash, nodesToHashInSent);

                AffiliateChannel affiliateChannel = (AffiliateChannel)context.Extra["informat"];

                if (buyer.AlwaysSoldOption == 0 && (PastSeconds >= buyerChannel.Timeout + buyerChannel.AfterTimeout || (affiliateChannel.Timeout.HasValue && affiliateChannel.Timeout.Value > 0 && PastTotalSeconds >= affiliateChannel.Timeout.Value)) && result.ErrorType != RequestResult.ErrorTypes.BuyerChannelResponseError)
                {
                    if (result.ResultType != RequestResult.ResultTypes.Success)
                    {
                        var res = result.ResultType;
                        ...
                        InsertPostedData(context, lead, leadContent, buyerChannel, "", RequestResult.ResultTypes.Reject);
                        InsertLeadMainResponse(context, buyerChannel, lead, leadContent, utcNow,
                            (Helpers.UtcNow() - utcNow).TotalMilliseconds, "Channel timeout", "Channel timeout", 0, 0,
                            RequestResult.ResultTypes.Reject, timeoutErrorType, null);
            ...
            else
            {
                if (result.ResultType != RequestResult.ResultTypes.Success)
                {
                    //context.StartDateUtc = DateTime.Now;
                    var res = result.ResultType;
                    var msg = result.Message;
                    ...
                    InsertLeadMainResponse(context, buyerChannel, lead, leadContent, utcNow,
                        (Helpers.UtcNow() - utcNow).TotalMilliseconds, "Channel timeout", "Channel timeout", 0, 0,
                        RequestResult.ResultTypes.Reject, RequestResult.ErrorTypes.RequestTimeoutError, null);
            ...
            Buyer buyer)
        {
            ...
            var customTimeZone = systemTimeZone != null ? systemTimeZone.BaseUtcOffset.TotalHours : 0;
                ...
                //(buyerChannel.TimeZone.HasValue ? buyerChannel.TimeZone.Value : 0);

            if (!string.IsNullOrEmpty(buyerChannel.Holidays))
            {
                var holidays = buyerChannel.Holidays.Split(',');
                created = context.SettingService.GetTimeZoneDate(createdOriginal, null, context.TimeZoneSetting, customTimeZone);
                foreach (var s in holidays)
                {
                    ...
                    if (
                        (created.Month == month && created.Day == day && !Adrack.Web.Helpers.Utils.IsHoliday(created)) || 
                        (Adrack.Web.Helpers.Utils.IsHoliday(created) && Adrack.Web.Helpers.Utils.IsFederalHoliday(created))
                        )
                    {
                        return -1;
                    }
            ...
            var schedule = (List<BuyerChannelSchedule>)context.BuyerChannelScheduleService.GetBuyerChannelsByBuyerChannelId(buyerChannel.Id);
            ...
            foreach (var s in schedule)
            {
                ...
                if (s.DayValue == (short)created.DayOfWeek + 1)
                {
                    if (!dateCalc)
                    {
                        dateCalc = true;
                        created = context.SettingService.GetTimeZoneDate(createdOriginal, null, context.TimeZoneSetting, customTimeZone);
                        //convert from UTC to system time zone
                        //because the reporting by hour table is in system timezone
                    }

                    if (s.Price.HasValue)
                        schedulePrice = (decimal)s.Price;
                    else
                        schedulePrice = 0;

                    var h = (double)Math.Floor((decimal)s.FromTime / 60);
                    if (Math.Abs(h - 24) < TOLERANCE) h = 0;
                    var span = TimeSpan.FromMinutes(s.FromTime);
                    var fromDate = new DateTime(created.Year, created.Month, created.Day, (int)h, span.Minutes,
                        span.Seconds);

                    h = (double)Math.Floor((decimal)s.ToTime / 60);
                    span = TimeSpan.FromMinutes(s.ToTime);
                    var hh = h;
                    if (h == 24) h = 0;

                    var toDate = new DateTime(created.Year, created.Month, created.Day, (int)h, span.Minutes,
                        span.Seconds);
                    if (hh == 24) toDate = toDate.AddDays(1);

                    //If fromDate and toDate are in buyer channel timezone, then convert to system time zone
                    if (customTimeZone != 0)
                    {
                    ...
                    if (campaign.CampaignType == (short)CampaignTypes.LeadCampaign)
                    {
                        //fromDate = context.SettingService.GetUTCDate(fromDate, null, context.TimeZoneSetting, (buyerChannel.TimeZone.HasValue ? buyerChannel.TimeZone.Value : 0));
                        //toDate = context.SettingService.GetUTCDate(toDate, null, context.TimeZoneSetting, (buyerChannel.TimeZone.HasValue ? buyerChannel.TimeZone.Value : 0));
                        //created = context.SettingService.GetUTCDate(created, null, context.TimeZoneSetting, (buyerChannel.TimeZone.HasValue ? buyerChannel.TimeZone.Value : 0));

                        if (created < fromDate || created > toDate) return -1;

                        var qty = context.LeadMainResponseService.GetLeadsCountByPeriod(buyerChannel.Id, fromDate, toDate,
                            (s.LeadStatus.HasValue ? s.LeadStatus.Value : (short)-1));

                        if (qty >= s.Quantity && s.Quantity > 0) return -1;

                        if (s.HourMax > 0)
                        {
                            var mqty = context.LeadMainResponseService.GetLeadsCountByPeriod(buyerChannel.Id,
                                created.AddHours(-1), created, (s.LeadStatus.HasValue ? s.LeadStatus.Value : (short)-1));

                            if (mqty > s.HourMax && s.HourMax > 0) return -1;
                        }

                        if (buyer.LastPostedSold != null)
                        {
                            ts = Helpers.UtcNow() - (DateTime)buyer.LastPostedSold;
                            if (ts.TotalSeconds < s.SoldWait && s.SoldWait > 0) return -1;
                        }

                        if (buyer.LastPosted != null)
                        {
                            ts = Helpers.UtcNow() - (DateTime)buyer.LastPosted;
                            if (ts.TotalSeconds < s.PostedWait && s.PostedWait > 0) return -1;
                        }
                    }
                }
            }
            ...
            AffiliateChannel affiliateChannel, LeadContent leadContent)
        {
            ...
            if (AffiliatePriceOption == (short)AffiliateChannelPriceMethods.TakeFromAffilate) //take from affilate channel
            {
                short affiliatePriceMethod = (affiliateChannel.AffiliatePriceMethod.HasValue ? affiliateChannel.AffiliatePriceMethod.Value : (short)0);
                ...
                switch (affiliatePriceMethod)
                {
                    ...
                    case (short)AffiliateChannelPriceMethods.Revshare: AffiliatePriceOption = (short)AffiliateChannelPriceMethods.Revshare; break;
                    case (short)AffiliateChannelPriceMethods.TakeFromAffilate:
                        affiliatePriceMethod = (context.Affiliate.DefaultAffiliatePriceMethod.HasValue ? context.Affiliate.DefaultAffiliatePriceMethod.Value : (short)0);
                        if (affiliatePriceMethod == (short)AffiliateChannelPriceMethods.Fixed)
                        {
            ...
            if (AffiliatePriceOption == (short)AffiliateChannelPriceMethods.Revshare)
            {
                ...
                if (buyerChannel.BuyerPriceOption==0)
                    if (leadContent.Minprice + affiliateChannel.NetworkMinimumRevenue > buyerChannel.BuyerPrice) return -1;

                if (buyerChannel.BuyerPrice - buyerChannel.BuyerPrice * affiliateChannel.NetworkTargetRevenue / 100 >=
                    leadContent.Minprice)
                    AffiliatePrice = buyerChannel.BuyerPrice * (100 - affiliateChannel.NetworkTargetRevenue) / 100;
                else
        ...
        private bool ProcessData(RequestContext context, LeadMain lead)
        {
            ...
            buyerChannels = GetBuyerChannels(context, lead, leadContent).ToList();
            ...
            while (buyerChannels.Count > 0)
            {
                ...
                {
                    /*if (cycleSize > 0)
                        if (SharedData.CheckBuyerChannelLeadsCount(lead.CampaignId, buyerChannel))
                            SharedData.DecrementBuyerChannelLeadsCount(lead.CampaignId, buyerChannel);*/
                    if (buyerChannels.Count > 0) 
                        buyerChannels.RemoveAt(0);
                    continue;
                }
                ...
                try
                {
                    ...
                    if (SuperTierChannels.Count > 0)
                    {
                        ...
                        if (representTierFound)
                        {
                            /*if (cycleSize > 0)
                                if (SharedData.CheckBuyerChannelLeadsCount(lead.CampaignId, buyerChannel))
                                    SharedData.DecrementBuyerChannelLeadsCount(lead.CampaignId, buyerChannel);*/
                            if (buyerChannels.Count > 0) //ignore represent tier
                                buyerChannels.RemoveAt(0);
                            continue;
                        }
                    }
                    ...
                    if (!context.MinProcessingMode && !ValidateCoolOffPeriod(context, obj, buyerChannel, buyer))
                        if (campaign.CampaignType == (short)CampaignTypes.LeadCampaign)
                        {
                            InsertPostedData(context, lead, leadContent, buyerChannel, "", RequestResult.ResultTypes.Reject);
                            InsertLeadMainResponse(context, buyerChannel, lead, leadContent, Helpers.UtcNow(), 0,
                                "Cool off period", "Cool off period", buyerChannel.BuyerPrice, buyerChannel.AffiliatePrice,
                                RequestResult.ResultTypes.ScheduleError, RequestResult.ErrorTypes.ScheduleCapLimit, null);

                            Result.Set(RequestResult.ResultTypes.Reject, RequestResult.ErrorTypes.ScheduleCapLimit,
                                "Lead was not sold in the marketplace");
                    ...
                    if (schedulePrice == -1)
                    {
                        if (campaign.CampaignType == (short)CampaignTypes.LeadCampaign)
                        {
                            InsertPostedData(context, lead, leadContent, buyerChannel, "", RequestResult.ResultTypes.ScheduleError);
                            InsertLeadMainResponse(context, buyerChannel, lead, leadContent, Helpers.UtcNow(), 0,
                                "Schedule cap limit", "Schedule cap limit", buyerChannel.BuyerPrice,
                                buyerChannel.AffiliatePrice, RequestResult.ResultTypes.ScheduleError,
                                RequestResult.ErrorTypes.ScheduleCapLimit, null);
                        }
                    ...
                    if (campaign.CampaignType == (short)CampaignTypes.LeadCampaign)
                    {
                        ...
                        if (AffiliatePrice == -1 && result.ResultType != RequestResult.ResultTypes.Success)
                        {
                            ...
                            result.Set(RequestResult.ResultTypes.Reject, RequestResult.ErrorTypes.None,
                                "Lead was not sold in marketplace!");

                            InsertPostedData(context, lead, leadContent, buyerChannel, "", RequestResult.ResultTypes.Reject);
                            InsertLeadMainResponse(context, buyerChannel, lead, leadContent, Helpers.UtcNow(), 0,
                                "Min price error", "Min price error", 0, 0, RequestResult.ResultTypes.Reject,
                                RequestResult.ErrorTypes.None, null);
                        ...
                        if (!doNotPresentPassed)
                        {
                            ...
                            InsertPostedData(context, lead, leadContent, buyerChannel, "", RequestResult.ResultTypes.Reject);
                            InsertLeadMainResponse(context, buyerChannel, lead, leadContent, Helpers.UtcNow(), 0,
                                dnpMessage, dnpMessage, 0, 0, RequestResult.ResultTypes.Reject,
                                RequestResult.ErrorTypes.None, null);
                ...
                catch (Exception pingTreeException)
                {
                    ...
                    buyerChannels.Remove(buyerChannel);
                }
            }//end while
